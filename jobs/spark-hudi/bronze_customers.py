"""
Spark Structured Streaming: Kafka → Hudi bronze.customers
...
CHECKPOINT_PATH = f"{CHECKPOINT_BASE}/bronze_customers_hudi"
TABLE_PATH      = "s3a://warehouse/hudi/bronze/customers"
...
def write_bronze_batch(batch_df, batch_id):
    print(f"[bronze-customers-hudi] batch {batch_id}: {batch_df.count()} rows")
    ...
    (
        batch_df.write
        .format("hudi")
        .option("hoodie.table.name", "bronze_customers")
...
def main():
    ...
    raw = (
        ...
        .option("kafka.group.id", "hudi-bronze-customers")
        .load()
