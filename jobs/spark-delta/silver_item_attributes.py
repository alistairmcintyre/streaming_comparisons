"""
Spark Structured Streaming: Delta bronze.item_attributes → Delta silver.item_attributes
"""
...
CHECKPOINT_PATH  = f"{CHECKPOINT_BASE}/silver_item_attributes_delta"
BRONZE_TABLE_PATH = "s3a://warehouse/delta/bronze/item_attributes"
SILVER_TABLE_PATH = "s3a://warehouse/delta/silver/item_attributes"


def upsert_to_silver(batch_df: DataFrame, batch_id: int):
    print(f"[silver-attr-delta] batch {batch_id}: {batch_df.count()} rows")
    if batch_df.rdd.isEmpty():
        print(f"[silver-attr-delta] batch {batch_id}: empty, skipping")
        return
    ...
    latest.createOrReplaceTempView("_silver_attr_delta_updates")

    spark.sql(f"""
        MERGE INTO delta.`{SILVER_TABLE_PATH}` AS t
        USING _silver_attr_delta_updates AS s
...
def main():
    spark = (
        ...
        .appName("silver-item-attributes-delta")
        .getOrCreate()
    )
