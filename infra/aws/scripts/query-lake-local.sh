...
if [ $# -gt 0 ]; then
  ...
  echo "scd2:   silver.accounts keeps EVERY version. effective_to / is_current are DERIVED:"
  echo "        SELECT *, LEAD(effective_from) OVER (PARTITION BY account_id"
  echo "                                             ORDER BY effective_from) AS effective_to"
  echo "        FROM delta_silver_accounts;   -- current row: effective_to IS NULL"
  echo "as-of:  the point of SCD2 — what was this account at the time of the trade:"
  echo "        SELECT t.*, a.country, a.tier FROM delta_gold_positions t"
  echo "        LEFT JOIN (<the view above>) a ON a.account_id = t.account_id"
