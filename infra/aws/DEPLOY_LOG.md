...
| 51 | `price` would land NULL even after #50 | `price` is NUMERIC in Postgres and Debezium's default `decimal.handling.mode=precise` emits base64 Connect Decimal bytes, while every job types price as DoubleType → set `decimal.handling.mode: double`. Would have silently poisoned the gold aggregate (net_notional = quantity × price) rather than failing loudly |
