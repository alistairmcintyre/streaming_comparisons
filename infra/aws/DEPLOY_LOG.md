...
| 55 | Hudi tables sync to Glue but EVERY Athena query fails `HIVE_UNKNOWN_ERROR` | Hudi 1.2.0 writes `hoodie.table.version=9` with `timeline.layout.version=2` (the LSM timeline under `.hoodie/timeline/`); Athena's Hudi reader expects the older flat timeline. Affects `_ro` AND `_rt`, so it is NOT the known "Athena cannot do MOR real-time" limitation. `hoodie.write.table.version=6` is delivered to the driver (verified in the mounted file) but IGNORED — Hudi still writes v9. Hudi data is correct and complete on S3 (DECIMAL preserved); it is simply not queryable from Athena. Query it via Spark — DuckDB has no Hudi reader either |
